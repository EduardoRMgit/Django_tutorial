    ...
    def mutate(self, info,
        ...
        if not ordenante.is_anonymous:
            ...
            if token:
                ...
                try:
                    institucionBeneficiariaInt = (
                        InstitutionBanjico.objects.get(
                            id=institucionBeneficiariaInt))
                except Exception:
    ...
    def mutate(self, info,
        ...
        if not user.is_anonymous:
            ...
            if token:
                ...
                # _valida(hoy.weekday() in range(5, 7),
                #         'No se opera fines de semana')
    ...
    def mutate(self,
        ...
        if not ordenante.is_anonymous:
            ...
            if token:
                ...
                validar(
                    not LimiteTrans(
                        ordenante.id).dep_efectivo_mes(float(monto)),
                    'Límite transaccional superado'
                )
                validar(
                    not LimiteTrans(
                        ordenante.id).dep_efectivo_dia(float(monto)),
                    'Límite transaccional superado'
                )
